...
void main() {
  ...
  setUpAll(() async {
    admin = await PostgresConnection.connect(pg.url);
  });
  ...
  group('PostgresDatabase.connect', () {
    test('rejects invalid pool options', () async {
      await expectLater(
        PostgresDatabase.connect(pg.url, maxConnections: 0),
        throwsArgumentError,
      );
    });
  ...
  group('concurrency', () {
    test(
      '30 concurrent queries share at most maxConnections backends',
      () async {
        final db = await PostgresDatabase.connect(pg.url, maxConnections: 3);
        try {
    ...
    test(
      'queries inside one transaction are serialized on one connection',
      () async {
        final db = await PostgresDatabase.connect(pg.url);
        try {
  ...
  group('transaction pinning', () {
    test(
      'uses one backend for the whole callback, another for outer queries',
      () async {
        final db = await PostgresDatabase.connect(pg.url);
        try {
    ...
    test('rolls back on error and keeps the connection', () async {
      ...
      final single = await PostgresDatabase.connect(pg.url, maxConnections: 1);
      try {
  ...
  group('broken connections', () {
    test(
      'an idle backend killed externally is replaced on next acquire',
      () async {
    ...
    test(
      'an in-use backend killed externally fails that call and is discarded',
      () async {
        final db = await PostgresDatabase.connect(pg.url, maxConnections: 1);
        try {
          await expectLater(
  ...
  group('session hygiene', () {
    test(
      'a connection left inside a transaction is discarded, not reused',
      () async {
        final db = await PostgresDatabase.connect(pg.url, maxConnections: 1);
        try {
  ...
  group('PostgresConnection state', () {
    test(
      'a server error leaves the connection usable and not broken',
      () async {
